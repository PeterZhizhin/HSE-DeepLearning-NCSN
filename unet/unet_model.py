...
class DownBlock(nn.Module):
    ...
    def __init__(self, input_ch, output_ch, kernel_size, block_depth):
        ...
        self.layers = nn.Sequential(
            nn.MaxPool2d(2),
            conv_block.ConvBlock(input_ch, output_ch, kernel_size, block_depth),
        )
...
class UpBlock(nn.Module):
    ...
    def __init__(self, input_ch, output_ch, kernel_size, block_depth):
        ...
        self.conv_block = conv_block.ConvBlock(input_ch + output_ch, output_ch, kernel_size, block_depth)
...
class UNet(nn.Module):
    def __init__(self, n_classes, feature_levels_num, input_ch_size,
        ...
        self.input_block = conv_block.ConvBlock(input_ch_size, hidden_ch_size, 1, block_depth)
        self.down_blocks = []
        self.up_blocks = []
        ...
        self.output_block = conv_block.ConvBlock(hidden_ch_size, hidden_ch_size,
                                                 kernel_size, output_block_depth)
    ...
    def forward(self, x):
        ...
        for i, up_block in enumerate(self.up_blocks[::-1]):
            ...
            x = up_block(correct_down_block_output, x)
