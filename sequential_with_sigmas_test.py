...
class TestSequentialWithSigmas(TestCase):
    def test_forward_returns_correct_shape(self):
        test_class = sequential_with_sigmas.SequentialWithSigmas(
            nn.Conv2d(1, 1, 1),
            cond_instance_norm_plus_plus.ConditionalInstanceNormalizationPlusPlus(2, 1),
            nn.Conv2d(1, 1, 1),
            cond_instance_norm_plus_plus.ConditionalInstanceNormalizationPlusPlus(2, 1)
        )
    ...
    def test_forward_calls_cond_inst_norm_pp_with_sigmas(self):
        cond_inst_norm_pp = cond_instance_norm_plus_plus.ConditionalInstanceNormalizationPlusPlus(2, 1)
